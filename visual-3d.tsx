...
const trailFs = `
  ...
  void main() {
    ...
    gl_FragColor = vec4(color, vAlpha * pow(1.0 - d * 2.0, 2.0));
  }
`;
...
export class AxeeVisuals3D extends LitElement {
  ...
  private initThree() {
    ...
    this.labelRenderer.setSize(this.clientWidth, this.clientHeight);
  ...
  private updateGalaxyVisuals() {
    ...
    // FIX: Explicitly type the Set to avoid inferring 'never' type.
    ...
    for (const sceneId of sceneGalaxyIds) {
      if (!currentGalaxyIds.has(sceneId)) {
        ...
        if (galaxyObj) {
          for (const child of [...galaxyObj.children]) {
            // FIX: Add a `typeof` check to ensure `child.userData.id` is a string.
            // This acts as a type guard, satisfying TypeScript's type checker and resolving
            // errors where an 'any' type from userData was passed to functions
            // expecting a 'string'.
            if (child.userData.type === 'planet' && child.userData.id && typeof child.userData.id === 'string') {
              this.removePlanetTrail(child.userData.id);
              this.objectMap.delete(child.userData.id);
              this.materialsMap.delete(`planet-${child.userData.id}`);
              this.materialsMap.delete(`atmosphere-${child.userData.id}`);
            }
          }
          this.galaxyGroup.remove(galaxyObj);
          this.objectMap.delete(sceneId);
          this.materialsMap.delete(`galaxy-${sceneId}`);
        }
      }
    }
  ...
  private createGalaxy(galaxyData: GalaxyData) {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        uTime: {value: 0},
        uSize: {value: GALAXY_PARTICLE_SIZE},
    ...
    this.materialsMap.set(`galaxy-${galaxyData.id}`, material);
  ...
  private updatePlanetVisuals(galaxyData: GalaxyData) {
    ...
    for (const [planetId, planetData] of galaxyData.planets) {
      let planetGroup = this.objectMap.get(planetId) as THREE.Group;
      ...
      if (!planetGroup) {
        ...
        this.materialsMap.set(`planet-${planetId}`, material);
        ...
        if (isHabitable) {
          ...
          const glowMat = new THREE.ShaderMaterial({
            ...
            vertexShader: atmosphereVs, // Reuse atmosphere VS, it provides necessary varyings.
            fragmentShader: `
              ...
              void main() {
                // A simple fresnel (rim lighting) effect.
        ...
        if (planetData.visualization.surfaceTexture !== 'GAS_GIANT') {
          ...
          this.materialsMap.set(`atmosphere-${planetId}`, atmosphereMat);
        }
        ...
        galaxyContainer.add(planetGroup);
        this.objectMap.set(planetId, planetGroup);
  ...
  private transitionView(oldFocus: FocusChangeEvent | undefined) {
    // 1. Stop any ongoing tweens to prevent conflicts
    if (this.cameraTween) {
    ...
    // 2. Calculate dynamic duration based on distance
    const distance = this.camera.position.distanceTo(targetPos);
    const duration = THREE.MathUtils.clamp(distance * 10, 1500, 3500);

    // 3. Disable controls during transition
    this.controls.enabled = false;

    // 4. Create new tweens with smoother easing and dynamic duration
    this.cameraTween = new TWEEN.Tween(this.camera.position)
      .to(targetPos, duration)
      .easing(TWEEN.Easing.Quintic.InOut) // Smoother easing
      .onComplete(() => {
        // Re-enable controls when camera tween is done
  ...
  private _renderLoop = () => {
    ...
    this.materialsMap.forEach((material) => {
      ...
      } else if (material.uniforms.uTime) {
      ...
      const galaxyData = this.galaxiesData.find(
        (g) =>
          ...
          (this.currentFocus.type === 'galaxy'
            ...
            : this.objectMap.get(this.currentFocus.id!)?.userData.galaxyId),
      );
      if (galaxyData) {
        this.updatePlanetVisuals(galaxyData);
      }
    ...
    if (this.currentFocus.type === 'planet' && this.currentFocus.id) {
      const planetGroup = this.objectMap.get(this.currentFocus.id);
      if (planetGroup) {
